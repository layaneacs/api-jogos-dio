...
namespace ApiCatalogoJogos.Controllers.V1
{
    ...
    public class CicloDeVidaIDController : ControllerBase
    {
        ...
        public Task<string> Get()
        {
            ...
            //-- acesse a rota - https://localhost:44304/api/v1/ciclodevidaid 
            stringBuilder.AppendLine($"Press F5 para ver que muda"); 
            stringBuilder.AppendLine($"Sempre o mesmo, em toda a execução da Aplicação");
            stringBuilder.AppendLine($"Singleton 1: {_exemploSingleton1.Id}");
            stringBuilder.AppendLine($"Singleton 2: {_exemploSingleton2.Id}");
            stringBuilder.AppendLine();
            stringBuilder.AppendLine($"Sempre o mesmo,a cada requisição");
            stringBuilder.AppendLine($"Scoped 1: {_exemploScoped1.Id}");
            stringBuilder.AppendLine($"Scoped 2: {_exemploScoped2.Id}");
            stringBuilder.AppendLine();
            stringBuilder.AppendLine($"Muda a cada chamada e a cada requisicão");
